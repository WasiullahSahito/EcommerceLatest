    ...
    <section className="bg-white py-3 lg:py-8 w-full">
      <div className="max-w-7xl mx-auto px-2 sm:px-4 lg:px-6">
          ...
          <div className="relative">
                  ...
                  return (
                    <div key={product.id} className="w-full md:w-1/2 lg:w-1/4 flex-shrink-0 px-2">
                      <div className="productItem shadow-md rounded-lg overflow-hidden border border-gray-100 hover:shadow-xl transition-shadow duration-300 bg-white">
                        <div className="group imgWrapper w-full overflow-hidden rounded-t-lg relative">
                          ...
                          <div className="actions absolute top-3 right-3 z-10 flex items-center gap-2 flex-col opacity-0 group-hover:opacity-100 transition-all duration-300">
                            <button
                              ...
                              className="w-9 h-9 rounded-full bg-white text-gray-700 flex items-center justify-center transition-colors shadow-md hover:scale-110"
                            >
                              <Heart
                                className={`w-4 h-4 transition-colors ${isWishlisted ? "text-primeColor fill-current" : ""
                                  }`}
                              />
                            </button>
                            <button
                        ...
                        <div
                          className="info p-4 relative bg-white cursor-pointer"
                          onClick={() => handleProductNavigation(product.id)}
                        >
                          <h6 className="text-sm text-gray-500 mb-1">{product.brand}</h6>
                          <h3 className="text-base font-semibold mb-2 text-gray-900 line-clamp-2 min-h-[3rem]">
                            {product.name}
                          </h3>
            ...
            {products.length > itemsPerView && (
              <>
                <button
                  onClick={prevSlide}
                  className="absolute left-0 top-1/2 transform -translate-y-1/2 bg-white/90 hover:bg-white text-gray-800 p-3 rounded-full shadow-lg transition-all duration-300 z-10 border border-gray-200"
                >
                  <ChevronLeft className="w-5 h-5" />
                </button>
                <button
                  onClick={nextSlide}
                  className="absolute right-0 top-1/2 transform -translate-y-1/2 bg-white/90 hover:bg-white text-gray-800 p-3 rounded-full shadow-lg transition-all duration-300 z-10 border border-gray-200"
                >
        ...
        <div className="flex justify-center mt-8 space-x-2">
          {Array.from({ length: totalSlides + 1 }).map((_, index) => (
            <button
              ...
              className={`w-2.5 h-2.5 rounded-full transition-all duration-300 ${currentSlide === index ? "bg-primeColor scale-125" : "bg-gray-300 hover:bg-gray-400"
                }`}
            />
          ))}
        </div>
        ...
        <div className="text-center mt-12">
          <Link
            to="/products"
            className="inline-block bg-primeColor text-white px-8 py-3 rounded-lg font-medium hover:bg-transparent hover:text-primeColor  hover:border hover:border-primeColor transition-colors duration-300 shadow-md hover:shadow-lg"
          >
            View All Products
          </Link>
        </div>
